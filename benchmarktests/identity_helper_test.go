...
func TestParseGroups(t *testing.T) {
	tests := []struct {
		...
		{name: "default is balanced", groups: nil, groupCount: 10, entityCount: 100, wantFilled: 10, wantSize: 10},
		{name: "balanced rounds up", groups: &GroupConfig{Preset: "balanced"}, groupCount: 7, entityCount: 100, wantFilled: 7, wantSize: 15},
		{name: "empty", groups: &GroupConfig{Preset: "empty"}, groupCount: 10, entityCount: 100, wantFilled: 0, wantSize: 0},
		{name: "full", groups: &GroupConfig{Preset: "full"}, groupCount: 10, entityCount: 100, wantFilled: 10, wantSize: 100},
		...
		{name: "preset and count conflict", groups: &GroupConfig{Preset: "balanced", Count: 5}, groupCount: 10, entityCount: 100, wantErr: true},
	}

	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			filled, size, err := parseGroups(tc.groups, tc.groupCount, tc.entityCount)
			if tc.wantErr {
				if err == nil {
