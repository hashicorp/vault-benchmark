...
function build() {
  ...
  # Build our ldflags
  msg="--> Building vault-benchmark revision $revision, built $build_date"
  ...
  ldflags="${ldflags} -X github.com/hashicorp/vault-benchmark/version.GitCommit=$revision -X github.com/hashicorp/vault-benchmark/version.BuildDate=$build_date"

  if [[ ${BASE_VERSION+x} ]]; then
    msg="${msg}, base version ${BASE_VERSION}"
    ldflags="${ldflags} -X github.com/hashicorp/vault-benchmark/version.Version=$BASE_VERSION"
  fi

  if [[ ${PRERELEASE_VERSION+x} ]]; then
    msg="${msg}, prerelease ${PRERELEASE_VERSION}"
    ldflags="${ldflags} -X github.com/hashicorp/vault-benchmark/version.VersionPrerelease=$PRERELEASE_VERSION"
  fi

  if [[ ${METADATA_VERSION+x} ]]; then
    msg="${msg}, metadata ${METADATA_VERSION}"
    ldflags="${ldflags} -X github.com/hashicorp/vault-benchmark/version.VersionMetadata=$METADATA_VERSION"
  fi
