...
.PHONY: image
image:
	CGO_ENABLED=0 GOARCH=$(GOARCH) GOOS=linux go build -a -ldflags $(LDFLAGS) -o dist/linux/$(GOARCH)/$(BIN_NAME) .
	docker build --platform linux/$(GOARCH) --build-arg VERSION=$(VERSION) --no-cache -t $(IMAGE_TAG) -t $(LATEST_TAG) .
